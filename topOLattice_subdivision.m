% topQuadtreeOC(32,16,0.4, 2,1,200)
function [V,E,A,L,xPhys,cList,vList]=topOLattice_subdivision(nelx,nely,volfrac, bSubdivision,bBlackWhite,ItMax)
...
% [VV, FF] = draw_frame(V, E, 0.5);
% saveOff(VV, FF, 'frame.off');
...
% F = sparse(2*(nely+1)*(nelx+1)-nely,1,-1,2*(nely+1)*(nelx+1),1); %test1
% F = sparse(2*(nely+1)*nelx+2,1,-1,2*(nely+1)*(nelx+1),1); %test2
...
%     dvx = ones(nely,nelx)/(nelx*nely*volfrac);
    ...
    dcxlinear = reshape(dcx',1,5*nelx*nely+nely);
    dvxlinear = reshape(dvx',1,5*nelx*nely+nely);
    for level = 1:nLevels
...
%         dfdx2 = 0*dfdx;
%         [df0dx,dfdx]=self_dc_dv(df0dx,dfdx,xPhys,nely,nx,ny,nLevels,nc,pNorm);
        iter = loopbeta;
        xval = hlinear;
        move = 0.01;
    ...
    if bBlackWhite
        if beta < 100 && (loopbeta >= 60 || change <= 0.0001)
            beta = 2*beta;
            loopbeta = 0;
            change = 1;
