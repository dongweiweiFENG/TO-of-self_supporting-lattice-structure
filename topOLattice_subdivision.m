...
while loop < ItMax && change>=0.0001
    ...
    [dcx,dvx]=self_filter_dc_dv(dcx,dvx,xPhys,nely,nx,ny,nLevels,nc,pNorm); 
    ...
    if MMA == 1
        ...
        xnew=self_filter(xnew,nely,nx,ny,nLevels,nc,pNorm);
...
%% PLOT DENSITIES
clearvars -except V E A L xPhys A0 cList vList sList
save 2d_subdivision;
A=A0*xPhys;
figure;
clf;
...
% initialize Extended Dependency
...
% translate the statevariables into struts density
...
function x_new=self_filter(xnew,nely,nx,ny,nlevels,nc,pNorm)
ii0=5*nely;
x_new=xnew;
for level=2:nlevels
%     hc=hp{level};
...
function [df0,dfv]=self_filter_dc_dv(df0dx,dfdx,xPhys,nely,nx,ny,nlevels,nc,pNorm)
ii0=5*nely;
df0=df0dx;
dfv=dfdx;
for level=2:nlevels
